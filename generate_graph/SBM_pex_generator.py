...
'''
def generate_change(inter_prob, intra_prob, alpha, p_ex):
    cps=[50]
    ...
    for t in range(maxt):
        if t < cps[0]:
            ...
            G_t = SBM_snapshot(G_t, 1.0, sizes, probs_2)
            G_times.append(G_t)
            print ("generating " + str(t), end="\r")

        else:
            G_t = SBM_snapshot(G_t, alpha, sizes, probs_2)
            G_times.append(G_t)
            print ("generating " + str(t), end="\r")
...
def generate_event(inter_prob, intra_prob, alpha, p_ex):
    cps=[50]
...
def main():
    ...
    if anomarly=='event':
        generate_event(inter_prob, intra_prob, alpha, p_ex)
    else:
        generate_change(inter_prob, intra_prob, alpha, p_ex)
